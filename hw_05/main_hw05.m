...
for timepoint = 1:1%number_of_timepoints
    ...
    if(number_svs_at_timepoint < 5)
        ...
        while(~(abs(xyz_del_0(1:3) - x_prev(1:3)) < 1))
            ...
            %row_of_A = 1;
            for kk = 1:number_svs_at_timepoint 
                sv = svs_at_timepoint(kk);
                ...
                ttx = time_s(timepoint) - row_C1(idx_svs_at_timepoint(kk))/c;
                ...
                % %jaX
                A(kk, 1:3) = (- residual./rho_0)';
                l(kk) = row_C1(idx_svs_at_timepoint(kk)) - rho_0 + (c * satpos(4));
                % 
                % A(row_of_A, 1:3) = (- residual./rho_0)';
                % l(row_of_A) = row_C1(idx_svs_at_timepoint(kk)) - rho_0 + (c * satpos(4));
                % %jl
                % A(row_of_A, 4) = ;
                %row_of_A = row_of_A + 1;
            ...
            if iteration == 1
                disp("A after first iteration")
                disp(A)
                disp("l after first iteration")
                disp(l)
            end
