...
for timepoint = 1:1%number_of_timepoints
    ...
    if(number_svs_at_timepoint < 5)
        ...
        iteration = 0;
        for row_of_A = 1:svs_at_timepoint
            sv = svs_at_timepoint(row_of_A);
            ...
            ttx = time_s(timepoint) - row_C1(idx_svs_at_timepoint)/c;
            while(~(abs(xyz_del_0(1:3) - x_prev(1:3)) < 1))
                ...
                satpos = get_satpos(ttx(row_of_A), sv(row_of_A), eph,3);
                ...
                l(row_of_A) =   row_C1(idx_svs_at_timepoint(row_of_A)) - rho_0 + (c * satpos(4));
                % %jl
                % A(row_of_A, 4) = ;
                %row_of_A = row_of_A + 1;
