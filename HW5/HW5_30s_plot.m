...
time_fmt = datetime(t(:,1), t(:,2), t(:,3), t(:,4), t(:,5), t(:,6));
...
for time_i=1:N
    ...
    [Y,M,DoM,DoY,GPSW,DoGPSW,SoGPSW,JD,DecY] = gpsdate(t(time_i,1)+2000,t(time_i,2),t(time_i,3),t(time_i,4),t(time_i,5),t(time_i,6))
    ...
    txyz(time_i,:) = [SoGPSW, X0_adj, Y0_adj, Z0_adj];
...
% Plot Dx
subplot(3, 1, 1);  
plot(time_fmt, txyz(:,2), 'b-', 'LineWidth', 2);
title('X');
xlabel('Time');
ylabel('Position (m)');
grid on;
...
% Plot Dy
subplot(3, 1, 2); 
plot(time_fmt, txyz(:,3), 'b-', 'LineWidth', 2);
title('Y');
xlabel('Time');
ylabel('Position (m)');
grid on;
...
% Plot Dz
subplot(3, 1, 3); 
plot(time_fmt, txyz(:,4), 'b-', 'LineWidth', 2);
title('Z');
xlabel('Time');
ylabel('Position (m)');
grid on;
